...
const getHourlyCount = (
  ...
  const assignCounts = (id: string, designation: DesignationwithSalary, value: number) => {
  ...
  const getDataOfDate = (date: string) => {
    ...
    departments.forEach((department) => {
      const f = timekeeper.filter((f) => filterByDate(f, '1', department.department, date));
      const hf = timekeeper.filter((f) => filterByDate(f, '0.5', department.department, date));
  ...
  departments.forEach((department) => {
    const f = timekeeper.filter((f) => filterByDate(f, '1', department.department));
    const hf = timekeeper.filter((f) => filterByDate(f, '0.5', department.department));
    department.designations.forEach((des) => {
      const gender = des.gender[0].toLowerCase();
      const f8: TimeKeeper[] = f.filter(
        (f) => filterByDate(f, '1', department.department, undefined, gender) && filterByShift(f, 8)
      );
      const f12 = f.filter(
        (f) => filterByDate(f, '1', department.department, undefined, gender) && filterByShift(f, 12)
      );
      const hf8 = hf.filter(
        (f) => filterByDate(f, '0.5', department.department, undefined, gender) && filterByShift(f, 8)
      );
      const hf12 = hf.filter(
        (f) => filterByDate(f, '0.5', department.department, undefined, gender) && filterByShift(f, 12)
      );
      ...
      const s = des.seperateSalary?.find((s) => s.contractorId === contractorId);
      if (s) {
        rate[id] = s.salary;
      } else {
        rate[id] = wrkhrs === 12 ? des.basicsalaryfor12hr : des.basicsalary;
      }
      ...
      totalManDayAmount[id] = getRoundOff(a8 * des.basicsalary + a12 * des.basicsalaryfor12hr);
      //   }

      assignCounts('mandaysamount', des, Number(_.get(totalManDayAmount, id, 0)));
