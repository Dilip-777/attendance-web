...
function ImportData({ departments, designations }: { departments: Department[]; designations: Designations[] }) {
  // on change states
  ...
  const importing = async (data: any) => {
    ...
    data.forEach((d: any, index: number) => {
      const department = departments.find((dept) => dept.department === d['Department']);
      const designation = designations.find((desc) => desc.designation === d['Designation']);
    ...
    const body = data.map((data: any) => {
      const department = departments.find((d) => d.department === data['Department']);
      const designation = designations.find((d) => d.designation === data['Designation']);
      // if (!department || !designation) {
      //   return null;
      // }
      return {
  ...
  return (
    <Stack direction="row" alignItems="center" spacing={2}>
      <Button disabled={loading} variant="contained" component="label">
        ...
        <input hidden key={key} type="file" className="form-control" onChange={handleFile} required />
      </Button>
